...
/**
 * Servlet that processes a request to generate n prime numbers, each with at
 * least m digits. It performs the calculations in a low-priority background
 * thread, returning only the results it has found so far. If these results are
 * not complete, it sends a Refresh header instructing the browser to ask for
 * new results a little while later. It also maintains a list of a small number
 * of previously calculated prime lists to return immediately to anyone who
 * supplies the same n and m as a recent completed computation.
 */
